...
public class AccountFacadeREST extends AbstractFacade<Account> {
    ...
    @GET
    @Path("findAllAccounts/{userId}")
    @Produces({MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON})
    public List<Account> findAllAccounts(@PathParam("userId") int userId) {
        em.getEntityManagerFactory().getCache().evictAll();
        TypedQuery<Account> query = em.createQuery("select a from Account a where a.userId.userId = :userId", Account.class);
